...
<div class="works-container">
	<iframe class="file-preview" src="../../files/assignment-questions/<?= $questionURL ?>"></iframe>
	...
	<div class="to-review-container">
		...
		<?php
			$sql = "SELECT workID, studentName FROM Works NATURAL JOIN Students WHERE assignmentID=? AND checkedStatus=0 ORDER BY submittedDate, studentName";
			$stmt = $conn->prepare($sql);
			$stmt->bind_param('i', $_GET['a']);
			$stmt->execute();
			$result = $stmt->get_result();
			...
			if ($result->num_rows == 0) {
				?>
					<script>
						document.querySelector('.no-work-to-review').innerHTML = "No Works to review!<br /><a clas='only-for-teachers' href='./remind-all/'>Remind missing students</a>";
					</script>
				<?php
			} else {
	...
	<div class="reviewed-container">
		...
		<?php
			$sql = "SELECT workID, studentName FROM Works NATURAL JOIN Students WHERE assignmentID=? AND checkedStatus=1 ORDER BY submittedDate, studentName";
			$stmt = $conn->prepare($sql);
			$stmt->bind_param('i', $_GET['a']);
			$stmt->execute();
			$result = $stmt->get_result();
